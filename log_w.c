...
static __forceinline void SetTime(wchar_t *const restrict buffer, uint64_t const *const restrict localNow)
{
    ...
    uint16_t length = UInt64ToWChar(timeFields.Year, timeStringBuffer - 15);
    switch (length)
    {
    case 4:
    ...
    length = UInt64ToWChar(timeFields.Month, timeStringBuffer - 15);
    if (length == 1)
    {
        buffer[6] = '0';
    ...
    length = UInt64ToWChar(timeFields.Day, timeStringBuffer - 15);
    if (length == 2)
    {
    ...
    length = UInt64ToWChar(timeFields.Hour, timeStringBuffer - 15);
    if (length == 2)
    {
    ...
    length = UInt64ToWChar(timeFields.Minute, timeStringBuffer - 15);
    if (length == 2)
    {
    ...
    length = UInt64ToWChar(timeFields.Second, timeStringBuffer - 15);
    if (length == 2)
    {
...
NtStatus ConsoleLogW(wchar_t const *const message, uint16_t messageLength, LogLevel logLevel, wchar_t const *const source, uint16_t sourceLength, Handle outputHandle)
{
    ...
    logLineBuffer[bufferOffset++] = '[';
    __movsb(logLineBuffer + bufferOffset, _logWords[logLevel], (uint64_t)_logWordLengthMap[logLevel] << 1);
    bufferOffset += _logWordLengthMap[logLevel];
    logLineBuffer[bufferOffset++] = ']';
    logLineBuffer[bufferOffset++] = '-';
    logLineBuffer[bufferOffset++] = '[';

    __movsb(logLineBuffer + bufferOffset, source, (uint64_t)sourceLength << 1);
...
MESSAGE_PADDING:
    ...
    __movsb(logLineBuffer + bufferOffset, message, (uint64_t)messageLength << 1);
