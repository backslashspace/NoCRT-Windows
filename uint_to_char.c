...
static const char_t DIGITS_LUTA[201] = "00010203040506070809101112131415161718192021222324252627282930313233343536373839404142434445464748495051525354555657585960616263646566676869707172737475767778798081828384858687888990919293949596979899";
...
uint16_t UInt64ToChar(uint64_t value, char_t *const buffer)
{
    ...
    while (value >= 100)
    {
        index = (value % 100) << 1;
        value /= 100;
        buffer[position] = DIGITS_LUTA[index + 1];
        buffer[position - 1] = DIGITS_LUTA[index];
        position -= 2;
    }
